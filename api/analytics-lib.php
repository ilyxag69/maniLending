...
function maniAnalyticsInsertEvents(PDO $pdo, array $events): int {
    ...
    foreach (array_slice($events, 0, 20) as $event) {
        ...
        $statement->execute([
            ...
            ':content' => maniAnalyticsCleanText($event['content'] ?? '', 100) ?: null,
            ':term' => maniAnalyticsCleanText($event['term'] ?? '', 100) ?: null,
            ':cta_location' => maniAnalyticsCleanText($event['cta_location'] ?? '', 64) ?: null,
            ':section_name' => maniAnalyticsCleanText($event['section'] ?? '', 64) ?: null,
            ':network' => maniAnalyticsCleanText($event['network'] ?? '', 32) ?: null,
            ':tone' => maniAnalyticsCleanText($event['tone'] ?? '', 24) ?: null,
            ':control_name' => maniAnalyticsCleanText($event['experiment'] ?? $event['control'] ?? $event['field'] ?? '', 32) ?: null,
            ':action_name' => maniAnalyticsCleanText($event['action'] ?? $event['share_target'] ?? $event['target'] ?? '', 32) ?: null,
