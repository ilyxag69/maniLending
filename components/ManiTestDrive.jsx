...
export default function ManiTestDrive() {
  ...
  const assistantDetail = useMemo(() => {
    ...
    const motivator = ["Начнём с одного простого изменения.", "Тебе не нужно менять всю жизнь — выберем лёгкие точки экономии.", "Даже небольшая корректировка даст заметный результат за год.", "Ты уже сделал важное — спокойно посмотрел на цифры.", "Сначала сохраним комфорт, затем уберём лишнее.", "Эти деньги можно направить на цель, которая действительно радует.", "Порядок начинается не с запретов, а с ясной картины.", "Каждая найденная утечка — дополнительная свобода для твоих планов."];
