...
function ManiMascot({ mode }) {
  const image = mode === "jester" ? "/assets/newmani/interactive/jester.png" : "/assets/newmani/interactive/motivator.png";
...
export default function ManiTestDrive() {
  ...
  const assistantMessage = useMemo(() => {
    ...
    if (annualLoss <= 70000) return `Я вижу, что сейчас уходит около ${annualLossFormatted} в год. Не переживай и не кори себя — это скрытые маркеры, которые трудно отследить вручную. Мы разберёмся вместе и шаг за шагом вернём полный контроль.`;
